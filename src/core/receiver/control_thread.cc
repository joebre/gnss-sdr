...
void ControlThread::apply_action(unsigned int what)
{
    ...
    switch (what)
        ...
        case 12:
            ...
            // place. force=true: an operator explicitly issuing a hotstart
            // wants the classification recomputed now, not deferred by
            // Tick()'s background-tick throttle (recompute_interval_s_,
            // the periodic data-changed check, ...) the way a routine idle
            // tick would be.
            if (flowgraph_->visibility_aware_search_enabled())
                {
                    flowgraph_->MaybeUpdateVisibility(true);
                }
            else
                {
        ...
        case 13:
            ...
            // no XML file on disk can be more current than live-demodulated data) and
            // only drop ephemeris, so satellites still get visibility-classified/
            // prioritized purely from almanac + position/time and have to
            // redemodulate their own ephemeris during acquisition/tracking. No XML
            // reload here -- deliberately: reloading from a file risks *regressing*
            // past whatever the receiver has already accumulated live since its last
            // start (see clear_ephemeris_keep_almanac()'s doc comment), and
            // bootstrapping a receiver that has never run at all is already handled
            // by the startup-time assist_GNSS() path, not this telecommand.
            //
            // Same choice HOTSTART makes just above, and for the same reason: when
            // the visibility-aware search is enabled, MaybeUpdateVisibility() already
            // supersedes the legacy one-shot reorder below -- take that path here too
            // instead of always falling back to it, otherwise a TC warmstart would
            // classify satellites differently (and reset the continuously-maintained
            // visible/maybe-visible buckets) compared to how the receiver would
            // otherwise be tracking that state. Warm and hot start now differ by
            // exactly the clear_ephemeris_keep_almanac() call above.
            pvt_ptr = flowgraph_->get_pvt();
            pvt_ptr->clear_ephemeris_keep_almanac();
            if (flowgraph_->visibility_aware_search_enabled())
                {
                    // force=true -- see HOTSTART's identical call above.
                    flowgraph_->MaybeUpdateVisibility(true);
                }
            else
                {

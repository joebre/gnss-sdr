...
public:
     ...
     * Called from ControlThread's idle-tick loop with the default \p force =
     * false; a HOTSTART/WARMSTART telecommand instead passes force = true
     * (see SatelliteVisibility::Tick()'s doc comment for why).
     */
    void MaybeUpdateVisibility(bool force = false);
...
private:
    ...
    // TEMPORARY DEBUG: primary-signal Doppler-predict acquisition assist is
    // computed but not applied (kApplyDopplerPredictAssist below) -- this
    // stashes the prediction per channel_id from acquisition_manager() so
    // apply_action()'s ACQ SUCCESS case (1) can log it against the Doppler
    // the regular full-range search actually found, to validate the
    // prediction's accuracy before trusting it to narrow the search again.
    // Erased once read (or if a channel gets a new attempt before success).
    std::map<unsigned int, double> d_predicted_doppler_hz_for_debug_;

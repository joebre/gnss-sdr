...
public:
     ...
     * valid, more ephemeris/almanac data arrived since the last check, the
     * recompute interval elapsed, or \p force is true. Uses the live PVT fix
     * when valid, else the static GNSS-SDR.AGNSS_ref_location/AGNSS_ref_utc_time
     * if both are explicitly configured, else does nothing (insufficient
     * information) -- see have_agnss_reference_'s doc comment for why an
     * unconfigured time never falls back to the host's wall-clock time here.
     *
     * \param force Skip all the throttle checks above and always do a full
     * recompute -- for callers where "not now" isn't an acceptable answer,
     * namely a HOTSTART/WARMSTART telecommand: an operator explicitly asking
     * the receiver to start now shouldn't have that silently deferred by a
     * background-tick throttle tuned for periodic idle-loop calls. Ordinary
     * periodic ticks should leave this false.
     *
     * \returns true iff bucket membership actually changed, i.e. the caller
     * should rebuild its per-signal search queues.
     */
    bool Tick(const std::shared_ptr<PvtInterface>& pvt_ptr, const Monitor_Pvt& fix_status, bool force = false);
    ...
    /*!
     * \brief Predicted Doppler (Hz) for sat on the given signal (e.g. "1B",
     * "5X", "L5", "7X", "E6", "B1"... -- anything in SIGNAL_FREQ_MAP,
     * gnss_frequencies.h, that also has a known Gnss_Ephemeris/
     * Gnss_Almanac::predicted_doppler() band code), for use as an
     * acquisition search center or as a diagnostic comparison against
     * another Doppler source (e.g. a dual-frequency projection).
     *
     * For a satellite's primary signal (band 1: GPS L1/Galileo E1/BeiDou B1
     * -- the one this class's own classification is driven by), the caller
     * would normally have already confirmed IsVisible(sat) first; for a
     * secondary signal there's no such precondition -- this just reports
     * whatever the ephemeris/almanac math says, regardless of the primary
     * signal's own visibility state.
     *
     * Computed from whichever of sat's ephemeris/almanac is currently
     * usable (ephemeris preferred, same freshness rules as
     * compute_visible_satellites()), evaluated at the same receiver
     * position/time Tick() itself would use (live PVT fix if valid, else
     * the static AGNSS reference), via Gnss_Ephemeris::predicted_doppler()/
     * Gnss_Almanac::predicted_doppler() -- the geometric term from
     * satellite-receiver relative motion. A local receiver clock drift term
     * (fix_status.user_clk_drift_ppm scaled to this signal's carrier, once a
     * live fix exists; GNSS-SDR.clock_frequency_offset_ppm before that) is
     * added on top: a common bias shared by every satellite's observed
     * Doppler, which the pure geometric prediction alone doesn't account
     * for.
     *
     * std::nullopt when disabled, no position/time source is available yet
     * (same conditions as Tick() returning false for that reason), signal
     * isn't a recognized carrier, or neither ephemeris nor almanac is
     * currently usable for sat (shouldn't happen for the primary signal
     * when IsVisible(sat) is true, since that requires one of them to have
     * driven the classification in the first place -- but data can expire
     * between Tick() calls; no such guarantee exists for a secondary
     * signal).
     */
...
private:
    ...
    // GNSS-SDR.clock_frequency_offset_ppm: static receiver clock drift
    // estimate, used as the clock-offset term in PredictedPrimaryDopplerHz()
    // only before a live PVT fix exists (fix_status.user_clk_drift_ppm --
    // PVT's own live-solved drift -- is meaningless/stale before that point,
    // see get_PVT()'s result==0 handling in rtklib_solver.cc). Default 0.0
    // (no assumed offset) reproduces today's no-fix-yet behavior exactly.
    ...
    // True only when BOTH GNSS-SDR.AGNSS_ref_location AND
    // GNSS-SDR.AGNSS_ref_utc_time are explicitly set and valid -- unlike
    // parse_agnss_ref_utc_time()'s own default (used as-is by
    // ControlThread::init()'s cold-start code), an empty/unset
    // AGNSS_ref_utc_time is deliberately NOT treated as "fall back to the
    // host's wall-clock time" here: wall-clock time has no relationship to
    // the GNSS time actually embedded in the samples being processed (most
    // acutely wrong for offline/replay runs, but also just an unforced,
    // avoidable guess for a live capture when the real reference is
    // knowable), so both the no-fix-yet elevation classification below and
    // PredictedDopplerHz() simply have no AGNSS reference at all rather than
    // silently computing against a guessed instant.
    bool have_agnss_reference_;
    double agnss_ref_lat_deg_;
    double agnss_ref_lon_deg_;
    // GNSS-SDR.AGNSS_ref_utc_time, only meaningful (and only set) when
    // have_agnss_reference_ is true -- see its doc comment above.

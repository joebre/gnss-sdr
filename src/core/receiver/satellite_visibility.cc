...
{
    // Location parsing is shared with ControlThread::init() (see
    // agnss_ref_location.h) -- no fallback either way, parsed_location.valid
    // is false whenever GNSS-SDR.AGNSS_ref_location is unset or malformed.
    ...
    // parse_agnss_ref_utc_time() itself treats an empty ref_time_str as an
    // "educated guess" case and returns the host's wall-clock time marked
    // valid (see its doc comment; ControlThread::init() relies on exactly
    // that for its own cold-start code) -- checking the raw property string
    // first, rather than trusting parsed_time.valid alone, is what tells an
    // explicitly-configured reference apart from that fallback here. See
    // have_agnss_reference_'s doc comment for why this class doesn't want it.
    const std::string ref_utc_time_str = configuration->property("GNSS-SDR.AGNSS_ref_utc_time", std::string(""));
    const Agnss_Ref_Time parsed_time = parse_agnss_ref_utc_time(ref_utc_time_str);
    const bool have_explicit_ref_time = !ref_utc_time_str.empty() && parsed_time.valid;

    have_agnss_reference_ = parsed_location.valid && have_explicit_ref_time;
    agnss_ref_utc_time_ = have_explicit_ref_time ? static_cast<time_t>(parsed_time.seconds) : 0;
...
bool SatelliteVisibility::Tick(const std::shared_ptr<PvtInterface>& pvt_ptr, const Monitor_Pvt& fix_status, bool force)
{
    if (!enabled_ || !pvt_ptr)
        {
            return false;
    ...
    if (!force && !fix_became_valid && !data_changed && !interval_elapsed && !moved_significantly && !expired)
        {
            return false;
        }
    ...
    const bool needs_full_recompute = force || fix_became_valid || interval_elapsed || moved_significantly || expired;
    ...
    // diagnosing CPU load.
    {
        ...
        std::sort(maybe_visible.begin(), maybe_visible.end(), [](const auto& a, const auto& b)
                { return a.get_PRN() < b.get_PRN(); });
        ...
        oss << "[visibility] recompute (triggered by:"
            ...
            << ", LLH " << LLH[0] << " deg, " << LLH[1] << " deg, " << LLH[2] << " m"
            // Same label and scale (seconds-of-GPS-week) as the [doppler
            // predict] lines' rx_time= -- previously this printed
            // gps_gtime.time (RTKLIB's raw absolute epoch, a completely
            // different scale), making it impossible to directly compare a
            // recompute's timestamp against when a specific satellite was
            // being searched without doing GPS-week arithmetic by hand.
            << ", rx_time=" << fix_status.RX_time
    ...
    const Monitor_Pvt& fix_status, const Gnss_Satellite& sat, const std::string& signal) const
{
    ...
    // own band codes (see gnss_ephemeris.cc), keyed by signal string the same
    // way gnss_frequencies.h's SIGNAL_FREQ_MAP already is -- reused below for
    // this signal's carrier frequency (the clock-offset term scales by it).
    ...
    // Same position/time resolution as Tick() (see its comments): live PVT
    // fix when valid, else the static AGNSS reference, else give up.
    const bool fix_valid = (fix_status.RX_time >= 0.0);
    double lat_deg;
    double lon_deg;
    double h_m;
    ...
    if (system == "GPS")
        {
            ...
            if (eph_it != eph_map.cend() && std::abs(DataAgeS(rx_time_s, eph_it->second.toe)) <= MAXDTOE)
                {
            ...
            else
                {
                    ...
                    if (alm_it != alm_map.cend() && std::abs(DataAgeS(rx_time_s, alm_it->second.toa)) <= almanac_max_age_s_)
                        {
    ...
    else if (system == "Galileo")
        {
            ...
            const auto eph_it = eph_map.find(prn);
            if (eph_it != eph_map.cend() && std::abs(DataAgeS(rx_time_s, eph_it->second.toe)) <= MAXDTOE_GAL)
                {
                    geometric_doppler_hz = eph_it->second.predicted_doppler(rx_time_s, lat_deg, lon_deg, h_m, ve_mps, vn_mps, vu_mps, band);
                    data_source = "ephemeris";
                }
    ...
    else if (system == "Beidou")
        {
            ...
            const auto eph_it = eph_map.find(prn);
            if (eph_it != eph_map.cend() && std::abs(DataAgeS(rx_time_s, eph_it->second.toe)) <= MAXDTOE_BDS)
                {
                    geometric_doppler_hz = eph_it->second.predicted_doppler(rx_time_s, lat_deg, lon_deg, h_m, ve_mps, vn_mps, vu_mps, band);
                    data_source = "ephemeris";
                }
    ...
    if (!geometric_doppler_hz.has_value())
        {
            LOG(INFO) << "[doppler predict] " << sat << " (" << signal << "): no usable ephemeris/almanac "
                      << "at rx_time=" << rx_time_s << " s";
            return std::nullopt;
        }

    // Local receiver clock drift: a bias shared by every satellite's
    // observed Doppler (the local oscillator running fast/slow shifts the
    // apparent carrier frequency of everything received), on top of the
    // per-satellite geometric term above. With a live fix, PVT's own solved
    // drift (fix_status.user_clk_drift_ppm) is used -- that's the actual
    // receiver clock right now. Before any fix exists, that field is
    // meaningless (see get_PVT()'s result==0 handling in rtklib_solver.cc,
    // which invalidates RX_time -- fix_valid is false here whenever it does
    // -- but doesn't zero user_clk_drift_ppm, so it can't be trusted as
    // "zero" either): use the static GNSS-SDR.clock_frequency_offset_ppm
    // estimate instead, defaulting to 0.0 (no assumed offset) if unset.
    // carrier_freq_hz (this signal's, from SIGNAL_FREQ_MAP) was already
    // resolved above, alongside band.
    //
    // SUBTRACTED, not added: empirically verified against the established
    // dual-frequency projection path (project_doppler() in gnss_flowgraph.cc,
    // which scales the primary signal's already-TRACKED Doppler -- geometric
    // and clock together, whatever their actual relationship is -- by this
    // signal's/primary's carrier ratio). Live comparisons across several
    // satellites matched geometric - clock_offset to within noise, while
    // geometric + clock_offset was consistently off by exactly 2x clock_offset.
    const double clock_drift_ppm = fix_valid ? fix_status.user_clk_drift_ppm : clock_frequency_offset_ppm_;
    const double clock_offset_hz = clock_drift_ppm * 1.0e-6 * carrier_freq_hz;
    const double total_hz = *geometric_doppler_hz - clock_offset_hz;

    LOG(INFO) << "[doppler predict] " << sat << " (" << signal << "): source=" << data_source << " rx_time=" << rx_time_s << " s"
              << " lat=" << lat_deg << " lon=" << lon_deg << " h=" << h_m
              << " geometric=" << *geometric_doppler_hz << " Hz clock_offset=" << clock_offset_hz
              << " Hz (" << (fix_valid ? "user_clk_drift_ppm=" : "clock_frequency_offset_ppm=") << clock_drift_ppm
              << ") total=" << total_hz << " Hz";
